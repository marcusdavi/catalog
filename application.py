...
def showAllCategories():
	...
	return render_template('categories.html', categories=categories, items=lastestItems)		
...
@app.route('/category/<string:category_name>/<string:item_name>/showItem', methods = ['GET','POST'])
#protect this route with a required login
def ShowItem(category_name, item_name):
	category = session.query(Category).filter_by(name=category_name).one()
	itemToShow = session.query(Item).filter_by(cat_id=category.id).filter_by(name=item_name).one()
	return render_template('showitem.html', category_name=category_name, item=itemToShow)		
...
def NewCategoryItem(category_name):
	if request.method == 'POST':
		category = session.query(Category).filter_by(name=category_name).one()
		newItem = Item(name = request.form['name'], description = request.form['description'], cat_id = category.id, user_id='1')
		session.add(newItem)
		session.commit()
...
def EditCategoryItem(category_name, item_name):
	category = session.query(Category).filter_by(name=category_name).one()
	editedMenuItem = session.query(Item).filter_by(cat_id=category.id).filter_by(name=item_name).one()
	...
	if request.method == 'POST':
		...
		return render_template('editcategoryitem.html', category_name = category_name, item_name = item_name, item = editedMenuItem)
...
def DeleteCategoryItem(category_name, item_name):
	category = session.query(Category).filter_by(name=category_name).one()
	itemToDelete = session.query(Item).filter_by(cat_id=category.id).filter_by(name=item_name).one()
	...
	if request.method == 'POST':
		...
		return render_template('deletecategoryitem.html', category_name = category_name, item_name = item_name, item = itemToDelete)
